...
public class MyHashSetIterator<E> implements Iterator<E> {
    ...
    public MyHashSetIterator(LinkedList<E>[] hashTable) {
        ...
        tableIndex = 0;
        listIndex = 0;
        while(true){  // 결국 시작 tableIndex를 지정해주기 위한 반복문이네
            // tableIndex에 해당하는 LinkedList가 있는지
            try {
    ...
    public E next() {
        ...
        // mull이 아니라면 다음으로 tableIndex, listIndex를 이동시킬 예정
        ...
        while(true){
            ...
            // hashtable[tableIndex]의 listIndex 위치에 값이 있는지 (없다면 다음 tableIndex로)
            try {
                hashTable[tableIndex].get(listIndex);
                break;
            } catch (Exception e) {
